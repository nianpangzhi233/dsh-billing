...
function apply(ctx, config) {
  const state = {
    ...
    officialBalance: { syncedAt: 0, ok: false, message: '尚未同步', total: null, granted: null, toppedUp: null, currency: '' },
  }
  ...
  function snapshot() {
    ...
    return {
      ...
      officialBalance: {
        syncedAt: state.officialBalance.syncedAt,
        ok: state.officialBalance.ok,
        message: state.officialBalance.message,
        total: state.officialBalance.total,
        granted: state.officialBalance.granted,
        toppedUp: state.officialBalance.toppedUp,
        currency: state.officialBalance.currency,
      },
      persist: {
        path: persist.path,
  ...
  async function loadPersisted() {
    try {
      ...
      if (data.config) {
        ...
        if (data.config.apiKey !== undefined) state.config.apiKey = String(data.config.apiKey || '')
  ...
  async function fetchOfficialBalance(key) {
    const apiKey = (key !== undefined && key !== null) ? String(key).trim() : state.config.apiKey
    const setResult = (r) => {
      state.officialBalance = { syncedAt: Date.now(), ok: !!r.ok, message: r.message || (r.ok ? '官方余额同步成功' : '同步失败'), total: r.total ?? null, granted: r.granted ?? null, toppedUp: r.toppedUp ?? null, currency: r.currency || '' }
      return snapshot()
    }
    if (!apiKey) {
      return setResult({ ok: false, message: '未配置 API Key（设置 → 计费 → 余额）' })
    }
    try {
      ...
      if (!res.ok || !data || !Array.isArray(data.balance_infos)) {
        let reason = 'HTTP ' + res.status
        if (res.status === 401) reason = 'API Key 无效（401）'
        else if (res.status === 403) reason = '无权限（403）'
        else if (res.status === 429) reason = '请求过于频繁（429，该接口限流约 1 次/分钟）'
        else if (data && data.error) reason = String(data.error.message || JSON.stringify(data.error)).slice(0, 200)
        return setResult({ ok: false, message: reason })
      }
      const info = data.balance_infos.find((i) => i && i.currency === 'CNY') || data.balance_infos[0] || {}
      const total = parseFloat(String(info.total_balance || '0'))
      // 锚定：初始余额 = 官方总余额 + 已消费，使显示余额恰好等于官方余额
      state.config.initialBalance = (Number.isFinite(total) ? total : 0) + totalCost()
      schedulePersist()
      return setResult({
        ok: true,
        total: total,
        granted: parseFloat(String(info.granted_balance || '0')),
        toppedUp: parseFloat(String(info.topped_up_balance || '0')),
        currency: info.currency || '',
        message: '已锚定官方余额（总余额，含赠金与充值）',
      })
    } catch (err) {
      const reason = err && err.name === 'AbortError' ? '请求超时（15s）' : '同步失败：' + (err && err.message ? err.message : String(err))
      return setResult({ ok: false, message: reason })
    }
  }
  ...
  function setConfig(args) {
    ...
    if (args.apiKey !== undefined) state.config.apiKey = String(args.apiKey || '').trim()
  ...
  const routes = [
    ...
    { kind: 'exact', path: '/api/dsh-billing/sync-official-balance', handler: async (req, res) => { if (!guard(req, res, 'POST')) return; const body = await readJsonBody(req); if (body && typeof body === 'object' && body.apiKey !== undefined) { state.config.apiKey = String(body.apiKey || '').trim(); schedulePersist() } writeJson(res, 200, await fetchOfficialBalance()) } },
  ]
  ctx.effect(() => {
