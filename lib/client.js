...
`
    ...
    const API = {
      ...
      syncOfficialBalance: '/api/dsh-billing/sync-official-balance',
    }
    ...
    function BillingSection() {
      ...
      const [obSyncing, setObSyncing] = React.useState(false)
      ...
      const syncOfficialBalance = async () => {
        setObSyncing(true)
        try {
          await apiPost(API.syncOfficialBalance, { apiKey: form.apiKey })
          await refreshState()
        } catch { /* noop */ }
        finally { setObSyncing(false) }
      }

      const off = snap.official
      const ob = snap.officialBalance
      ...
      return React.createElement('div', { className: 'dbill-section' },
        React.createElement(Group, { title: '余额', defaultOpen: true },
          React.createElement('div', { className: 'dbill-sync-row' },
            React.createElement('input', { className: 'dbill-input', type: 'password', placeholder: 'DeepSeek API Key（sk-…）', value: form.apiKey, onChange: (e) => set('apiKey', e.target.value), style: { flex: '1', minWidth: '140px' } }),
            React.createElement('button', { className: 'dbill-btn dbill-btn-primary', onClick: syncOfficialBalance, disabled: obSyncing }, obSyncing ? '同步中…' : '同步官方余额'),
          ),
          ob ? React.createElement('span', { className: 'dbill-sync-status', style: { color: ob.ok ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-state-warn-primary)' } },
            (ob.ok ? '官方余额 ¥' + money(ob.total) + (ob.currency ? ' ' + ob.currency : '') + '（赠金 ¥' + money(ob.granted) + ' / 充值 ¥' + money(ob.toppedUp) + '）' : ob.message) + (ob.syncedAt > 0 ? ' · ' + dateTimeText(ob.syncedAt) : ''),
          ) : null,
          React.createElement('div', { className: 'dbill-note' }, '在 platform.deepseek.com → API Keys 生成 sk- 开头的 Key 填入上方，点击「同步官方余额」自动拉取并锚定余额（官方接口限流约 1 次/分钟）。也可手动粘贴锚定：'),
          React.createElement('div', { className: 'dbill-sync-row' },
            ...
            React.createElement('button', { className: 'dbill-btn', onClick: applyBalance }, balanceSynced ? '✓ 已同步' : '应用'),
          ),
